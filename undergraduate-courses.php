...
try {
    $stmt = $conn->prepare("
        ...
        WHERE p.published = 1 AND p.type = 'Undergraduate'
    ");
    $stmt->execute();
    $programs = $stmt->fetchAll(PDO::FETCH_ASSOC); 

    if (empty($programs)) {
        throw new Exception("No undergraduate programs found in the database.");
    }
...
<section class="header"> 
    ...
    <section class= "ug-banner">
    <header>
        <h1>Undergraduate Courses</h1>
        <nav>
            <a href="index.html">Home</a> / <a href="undergraduate-courses.php">Undergraduate Courses</a>
        </nav>
    </header>
    </section>
    <main>
            ...
            <section class="course-container">
                <?php foreach ($programs as $program): ?>
                    ...
                    <div class="course-info">
                            <p>👤 <?= htmlspecialchars($program['leader_name'] ?? 'N/A'); ?> | 🪑 10 Seats | ⏳ <?= htmlspecialchars($program['duration_years']); ?> Years</p>
        ...
        <?php else: ?>
            <p>No undergraduate programs available at the moment.</p>
        <?php endif; ?>
    </main>
