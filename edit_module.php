...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $conn->prepare("UPDATE modules 
    SET year = :year, block = :block, module_name = :module_name, module_leader_id = :module_leader_id, module_type = :module_type 
    WHERE id = :id");
    ...
    if ($stmt->execute()) {
        header("Location: admin_dashboard.php?message=Module updated successfully.");
        exit();
    } else {
...
<body>
    ...
    <img src="images/dmu white logo.png" alt="DMU Logo" class="dmu-logo">
        ...
        <div class="edit-container">
            <form class="edit-form" action="edit_module.php" method="POST">
                <h1>Edit Module</h1>
                ...
                <select name="module_type" required>
                    <option value="Mandatory" <?= $module['module_type'] === 'Mandatory' ? 'selected' : ''; ?>>Mandatory</option>
                    <option value="Elective" <?= $module['module_type'] === 'Elective' ? 'selected' : ''; ?>>Elective</option>
                </select>

                <label for="year">Year:</label>
                <input type="number" name="year" value="<?= htmlspecialchars($module['year']); ?>" required>

                <label for="block">Block:</label>
                <input type="text" name="block" value="<?= htmlspecialchars($module['block']); ?>" required>
                ...
                <select name="module_leader_id" required>
                    ...
                    <?php foreach ($staff_members as $staff): ?>
                        <option value="<?= $staff['staff_id']; ?>" 
                            <?= $module['module_leader_id'] == $staff['staff_id'] ? 'selected' : ''; ?>>
